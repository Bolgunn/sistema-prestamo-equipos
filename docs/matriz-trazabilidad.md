...
Respaldo de ejecución: `.venv/Scripts/pytest.exe -q` sobre `3b02603` arroja
**302 passed, 1 xfailed**. El único `xfail` es PC20-04, que corresponde al
defecto abierto DEF-02 (#47).
...
| RF-09 | Una solicitud aprobada puede pasar a entregada; una solicitud rechazada, cancelada, devuelta o sin aprobar no puede entregarse. | RN-13 | `src/prestamos/servicios/prestamos.py:40` (`ServicioPrestamos.registrar_entrega`) (orquesta), `src/prestamos/reglas.py:664` (`_validar_entrega`) (guarda) | CP-02, CP-15 | Verificado |
...
| RF-13 | Login exitoso/fallido, creación, aprobación, rechazo, entrega, devolución, cancelación y errores quedan registrados sin incluir contraseñas. | RN-18 | `src/prestamos/logging_conf.py:47` (`registrar_evento`) (orquesta), `src/prestamos/logging_conf.py:70` (`sanitizar`) (guarda) | CP-32, CP-33, CP-34, PC20-04 (XFAIL) | Verificado con defecto (DEF-02 / #47) |
...
4. **Identificadores no documentados en `docs/casos-de-prueba.md`.** CX01 a CX05
   (pruebas cruzadas del issue #21, en `tests/cruzadas/test_cruzadas_benjamin.py`)
   y CP-32 a CP-34 (observabilidad de RF-13, en
   `tests/funcionales/test_observabilidad.py`) se ejecutan y pasan, pero no tienen
   fila en la tabla de casos de prueba. Se citan aquí porque el requisito de no
   dejar pruebas huérfanas lo exige; documentarlos corresponde a los issues de
   diseño de casos, no a esta matriz.

5. **Casos mencionados sin implementación.** `docs/reglas-negocio.md` menciona
   CP-29 a CP-34; de esos, solo CP-32, CP-33 y CP-34 existen como prueba
   ejecutable. CP-29, CP-30 y CP-31 no tienen test asociado y por lo tanto no
   aparecen en ninguna fila.
...
7. **Estado de verificación.** `Verificado` significa que existe al menos una
   prueba ejecutada y en verde que cubre el criterio de aceptación.
   `Verificado con defecto` significa que el requerimiento está cubierto pero
   tiene un defecto abierto asociado. Sólo RF-13 está en esa situación: la
   entrega no registra el evento de auditoría que exige RN-18, lo que PC20-04
   reproduce como `XFAIL` y `docs/defectos.md` registra como DEF-02 (#47).
