...
| --- | --- | --- | --- | --- | --- | --- |
| RN-01 | Solo existen dos roles operativos: solicitante y encargado. | Al registrar, autenticar o autorizar una operación. | Si el rol no es solicitante o encargado, la operación se rechaza. | AMB-02 / RF-01 / RF-02 | `src/prestamos/auth.py`, `src/prestamos/reglas.py` | CP-01, CP-11, CP-15 |
| RN-02 | Solo usuarios activos pueden iniciar sesión y operar en el sistema. | Al iniciar sesión o ejecutar una acción autenticada. | Usuarios inactivos o inexistentes no pueden acceder ni crear operaciones. | AMB-01 / AMB-12 / RF-02 | `src/prestamos/auth.py` | CP-01, CP-15, CX01 |
| RN-03 | Todo usuario debe tener ID único, nombre, correo institucional con forma `nombre@dominio`, rol, estado y contraseña. | Al registrar o actualizar usuarios. | Si falta un campo obligatorio, si el correo no tiene forma válida, o si el ID o el correo ya existen —comparados sin distinguir mayúsculas—, el registro se rechaza. | AMB-01 / RF-01 | `src/prestamos/servicios/usuarios.py` | CP-15 |
| RN-04 | Todo equipo debe tener código único, nombre, tipo, descripción breve y estado operativo. | Al registrar o actualizar equipos. | Si falta un campo obligatorio o el código ya existe —comparado sin distinguir mayúsculas, incluidos los equipos dados de baja—, el registro se rechaza. | AMB-03 / RF-03 | `src/prestamos/servicios/equipos.py` | CP-15, CX03, CX04 |
| RN-05 | Solo equipos disponibles pueden ser solicitados o prestados. | Al crear o aprobar una solicitud. | Equipos en mantención, dados de baja, reservados o prestados no pueden agregarse a una nueva solicitud disponible para el mismo período. | AMB-04 / RF-04 / RF-05 / RF-08 | `src/prestamos/reglas.py` (guarda), `src/prestamos/servicios/solicitudes.py` (la dispara al crear y aprobar) | CP-08, CP-15, CX02 |
| RN-06 | Una solicitud debe incluir entre 1 y 3 equipos. | Al crear una solicitud. | Solicitudes sin equipos o con más de 3 equipos se rechazan. | AMB-05 / RF-05 | `src/prestamos/modelos.py` (invariante estructural), `src/prestamos/reglas.py` (revalida al crear) | CP-09, CP-15 |
| RN-07 | Un solicitante no puede tener más de 3 equipos activos al mismo tiempo. | Al crear o aprobar una solicitud. | Si al sumar reservas aprobadas y préstamos vigentes se supera el límite, la operación se rechaza. | AMB-05 / RF-07 | `src/prestamos/reglas.py` | CP-06, CP-13, CP-14 |
| RN-08 | La duración máxima de un préstamo es de 5 días hábiles. | Al crear o aprobar una solicitud con fecha de inicio y término. | Si el período solicitado supera 5 días hábiles, la solicitud se rechaza. | AMB-06 / RF-06 | `src/prestamos/reglas.py` | CP-09 y pruebas unitarias de `tests/unidad/test_reglas.py` |
| RN-09 | Las reservas futuras solo pueden solicitarse desde el día actual y hasta 20 días laborales hacia el futuro. | Al crear una solicitud con fecha de inicio futura. | Si la fecha de inicio está en el pasado o excede 20 días laborales, la solicitud se rechaza. | AMB-07 / RF-06 | `src/prestamos/reglas.py` | CP-09, CP-15 y pruebas unitarias de `tests/unidad/test_reglas.py` |
| RN-10 | No se puede aprobar una solicitud si existe solapamiento de fechas con otra reserva aprobada, préstamo entregado o préstamo atrasado del mismo equipo. | Al aprobar una solicitud o consultar disponibilidad. | El sistema considera el equipo no disponible para el período solicitado. | AMB-08 / RF-04 / RF-08 | `src/prestamos/reglas.py`, `src/prestamos/servicios/prestamos.py` | CP-08, CX02 |
| RN-11 | Solo el encargado puede aprobar o rechazar solicitudes. | Al intentar aprobar o rechazar una solicitud. | Si el usuario no tiene rol encargado, la operación se rechaza. | AMB-02 / AMB-08 / RF-08 | `src/prestamos/reglas.py` (`_validar_usuario_operador`), `src/prestamos/servicios/solicitudes.py` | CP-11 |
| RN-12 | Una solicitud solo puede aprobarse o rechazarse si está en estado solicitada. | Al aprobar o rechazar solicitudes. | Solicitudes aprobadas, rechazadas, canceladas, entregadas, devueltas o atrasadas no pueden aprobarse/rechazarse nuevamente. | AMB-09 / RF-08 | `src/prestamos/reglas.py` | Pruebas unitarias de `tests/unidad/test_reglas.py` y `tests/unidad/test_servicio_solicitudes.py` |
| RN-13 | La entrega solo puede registrarse sobre solicitudes aprobadas. | Al registrar entrega. | La solicitud pasa a estado entregada; si no estaba aprobada, la operación se rechaza. | AMB-09 / RF-09 | `src/prestamos/servicios/prestamos.py` | CP-02, CP-15, PC20-04 |
| RN-14 | La devolución solo puede registrarse sobre préstamos entregados o atrasados. | Al registrar devolución. | La solicitud pasa a estado devuelta y los equipos quedan liberados para nuevas solicitudes. | AMB-09 / RF-10 | `src/prestamos/servicios/prestamos.py` | CP-03, CP-07, CP-10, CP-15, PC20-01 |
| RN-15 | Una solicitud puede cancelarse solo antes de la entrega. | Al cancelar una solicitud. | Solicitudes solicitadas o aprobadas pasan a cancelada; préstamos entregados, atrasados o devueltos no pueden cancelarse. | AMB-10 / RF-11 | `src/prestamos/servicios/prestamos.py` | CP-04, CP-14 |
| RN-16 | Los préstamos se clasifican como futuros, vigentes o atrasados según estado y fecha actual. | Al consultar préstamos. | Futuro: aprobado con fecha de inicio posterior a hoy. Vigente: entregado y dentro del plazo. Atrasado: entregado con fecha de término vencida. | AMB-11 / RF-12 | `src/prestamos/servicios/prestamos.py` | CP-05, CP-13, CP-15, PC20-02 |
| RN-17 | El sistema debe validar entradas inválidas antes de persistir cambios. | Al recibir datos desde CLI o servicios. | Campos vacíos, fechas inválidas, IDs inexistentes, roles inválidos, estados inválidos y duplicados se rechazan con mensaje claro. | AMB-13 / RF-01 / RF-03 / RF-05 | `src/prestamos/cli.py`, `src/prestamos/reglas.py` | CP-12, CP-32 |
| RN-18 | Los eventos relevantes se registran en logs sin incluir contraseñas. | Al iniciar sesión, crear solicitudes, aprobar, rechazar, entregar, devolver, cancelar o detectar errores. | Se registra el evento y contexto no sensible; las contraseñas y secretos no se escriben en logs. | AMB-14 / RF-13 | `src/prestamos/logging_conf.py`, `src/prestamos/observabilidad.py`, `src/prestamos/servicios/prestamos.py` | CP-32, CP-33, CP-34, PC20-04 |
| RN-19 | El Solicitante solo puede consultar préstamos propios y el Encargado puede consultar todos, con filtros por usuario o equipo. | Al consultar préstamos futuros, vigentes o atrasados. | Si un solicitante intenta consultar préstamos de otro usuario, la operación se rechaza; el encargado puede consultar todos los préstamos o aplicar filtros por usuario/equipo. | AMB-02 / AMB-11 / RF-12 | `src/prestamos/servicios/prestamos.py` | CP-05, PC20-02, PC20-03 |
| RN-20 | El sistema debe conservar siempre al menos un Encargado activo. | Al desactivar un usuario o al cambiar su rol. | Si la operación dejaría al sistema sin ningún Encargado activo, se rechaza indicando que debe registrarse o reactivarse otro Encargado primero. | AMB-01 / AMB-02 / RF-01 | `src/prestamos/servicios/usuarios.py` | Pruebas unitarias de `tests/unidad/test_servicio_usuarios.py` y CX05 |
| RN-21 | Un equipo comprometido por un préstamo activo no puede darse de baja ni enviarse a mantención. | Al dar de baja o enviar a mantención un equipo. | Si existe un préstamo aprobado, entregado o atrasado que incluye el equipo, la operación se rechaza; una solicitud aún no aprobada no bloquea. | AMB-03 / AMB-04 / RF-03 | `src/prestamos/servicios/equipos.py` | Pruebas unitarias de `tests/unidad/test_servicio_equipos.py`, CX02, CX03, CX04 |
| RN-22 | Nadie puede aprobar su propia solicitud. | Al aprobar una solicitud. | Si quien aprueba es la misma persona que figura como solicitante, la operación se rechaza. Con roles fijos el caso no se alcanza —crear exige Solicitante y aprobar exige Encargado—, pero un cambio de rol (RN-20) puede promover a quien ya tenía una solicitud abierta. El rechazo no está cubierto: rechazar lo propio equivale a cancelarlo, que RN-15 ya permite. | AMB-02 / AMB-08 / RF-08 | `src/prestamos/reglas.py` | CX05 y `tests/unidad/test_reglas.py::test_nadie_aprueba_su_propia_solicitud` |
...
- Baja lógica reversible de usuarios y equipos, conservando su historial.
...
| SUP-16 | Las bajas de usuarios y equipos son lógicas y reversibles: nunca se borra el registro. | Resuelve RN-20 y RN-21 conservando el historial, porque `Prestamo` guarda el id del solicitante y los códigos de equipo como texto suelto y un borrado los dejaría huérfanos. | Si el cliente exige borrado real, habría que resolver primero qué hacer con los préstamos históricos. |
| SUP-17 | Los códigos de equipo no se reutilizan, ni siquiera tras una baja. | El código es la etiqueta física del activo; reutilizarlo reengancharía el historial de préstamos del equipo antiguo a un objeto distinto. | Si el laboratorio reetiqueta equipos, habría que permitir reutilización con una regla de desambiguación. |
| SUP-18 | Una solicitud aún no aprobada no impide retirar un equipo del catálogo. | Distingue promesa de petición: si un equipo se rompe, el Encargado debe poder retirarlo sin que una solicitud abierta lo bloquee. La solicitud se rechaza después en la aprobación, que revalida RN-05. | El solicitante no se entera hasta la aprobación; si el cliente exige aviso inmediato, haría falta notificación. |
| SUP-19 | No se define un dominio institucional concreto para el correo: solo se exige la forma `nombre@dominio`. | Ningún documento del cliente define el dominio, y fijarlo aquí inventaría una regla no acordada. | Si el cliente define un dominio, correos de otros dominios deberían rechazarse y habría que ajustar RN-03. |
